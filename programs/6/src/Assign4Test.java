...
public class Assign4Test extends TestCase {
 ...
 /** The following 9 check methods create an interpreter object with the specified String as the program, invoke the
  ...
  public void testLetRec() {
    try {
      String output = "(1 2 3 1 2 3)";
      String input = "letrec append := map x,y to if x = null then y else cons(first(x), append(rest(x), y)); " +  
                     "in let l := cons(1,cons(2,cons(3,null))); in append(l,l)";
  ...
  public void testProblem2() {
    try {
      String output = "(100)";
      String input = "letrec maplist := map f,s to if null?(s) then null else cons(f(first(s)), maplist(f, rest(s)));" +
                     "in maplist(map x to x*x, maplist(map x to 1000/x, cons(100, null)))";
